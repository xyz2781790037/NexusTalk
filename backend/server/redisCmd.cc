    ...
    if (!redisClient.is_connected()) {
        redisClient.connect(
            ...
            [](const std::string&, size_t,
               cpp_redis::client::connect_state state) {
...
std::string redisCmd::getField(const std::string& account,
...
void redisCmd::reviseData(std::string key,
                          std::string type,
                          std::string typedata) {
    LOG_INFO << "key= " << key;
...
void redisCmd::storeMessages(std::string sender,
                             std::string account,
                             std::string message) {
    connect();
    ...
    LOG_INFO << sender << " " << account;
    {
        ...
        redisClient.hset("frie:" + sender.substr(5), account,
                         std::to_string(count));
        redisClient.sync_commit();
    }
}
void redisCmd::sendOfflineMeg(nlohmann::json& data,
    ...
    for (int i = 0; i < count; i++) {
        std::string mesg =
            getRedisResult("offl:" + key.substr(5), field + std::to_string(i))
                .as_string();
        conn->send(MessageSplitter::encodeMessage(mesg));
        std::string key1 = tool::swapsort(key, field, "read:");
        messageQueue[key1].push_back(mesg);
        storeReadMeg(messageQueue[key1], key1);
        LOG_INFO << "删除";
...
void redisCmd::storeReadMeg(std::vector<std::string> messages,
                            std::string key) {
    LOG_DEBUG << "储存新消息";
    redisClient.rpush(key, messages);
    redisClient.sync_commit();
}
void redisCmd::sendHistoryMeg(nlohmann::json& data,
                              const TcpConnectionPtr conn) {
    int count = 0;
    std::string key = data["account"];
...
void redisCmd::sendHisOffineMeg(nlohmann::json& data,
                                const TcpConnectionPtr& conn) {
    std::string key = data["name"];
    std::string field = data["account"];
    LOG_INFO << "key=" << key << " field=" << field;
    int count =
        getRedisCount("frie:" + key.substr(5), "user:" + field.substr(5));
    LOG_INFO << "count=" << count;
    for (int i = 0; i < count; i++) {
        std::string mesg =
            getRedisResult("offl:" + key.substr(5),
                           "user:" + field.substr(5) + std::to_string(i))
                .as_string();
...
void redisCmd::joinGroup(std::string key,
                         std::string account,
                         std::string rank) {
...
void redisCmd::getGroupMeg(std::string key,
                           std::string name,
                           const TcpConnectionPtr conn,
                           int amount) {
    std::string sum;
...
void redisCmd::delPerson(std::string name,
                         std::string account,
                         std::string rank) {
    if (rank == "owner") {
